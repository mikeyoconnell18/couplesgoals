...
test('daily action honors inclusive dates in couple time zone', () => {
  ...
  assert.equal(
    isActionDue(action, new Date('2026-09-15T02:00:00Z'), 'America/New_York'),
    true,
  );
  assert.equal(
    isActionDue(action, new Date('2026-09-15T05:00:00Z'), 'America/New_York'),
    false,
  );
});
...
test('selected weekday is evaluated in couple time zone', () => {
  assert.equal(
    isActionDue(
      { cadence: 'weekdays', selectedWeekdays: [1], startDate: '2026-09-01' },
